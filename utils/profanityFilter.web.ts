// Web version - uses a simple word list instead of bad-words package
// The bad-words package has module compatibility issues on web

// Simple profanity filter for web
class SimpleProfanityFilter {
  ...
  constructor() {
    ...
    this.profaneWords = new Set([
      // Add common profane words here if needed
      // For now, keeping it minimal for the web version
    ]);
  }
  ...
  clean(text: string): string {
    ...
    return words.map(word => {
      return this.profaneWords.has(word.toLowerCase()) ? '****' : word;
    }).join(' ');
  }
  
  addWords(...words: string[]): void {
    words.forEach(word => this.profaneWords.add(word.toLowerCase()));
  }
  
  removeWords(...words: string[]): void {
    words.forEach(word => this.profaneWords.delete(word.toLowerCase()));
  }
}
